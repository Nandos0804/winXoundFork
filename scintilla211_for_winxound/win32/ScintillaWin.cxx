...
/// Implement IDropTarget
bool hasNameOfFile = false; //ADDED FOR WINXOUND
STDMETHODIMP ScintillaWin::DragEnter(LPDATAOBJECT pIDataSource, DWORD grfKeyState,
	...
	//MODDED FOR WINXOUND (//MK_CONTROL //MK_RBUTTON //MK_ALT)
...
STDMETHODIMP ScintillaWin::DragOver(DWORD grfKeyState, POINTL pt, PDWORD pdwEffect) {
	try {
		//MODDED FOR WINXOUND
		if (grfKeyState == MK_RBUTTON) {} else 
		//END_MOD
...
STDMETHODIMP ScintillaWin::Drop(LPDATAOBJECT pIDataSource, DWORD grfKeyState,
	...
	try {
		...
		if(hasNameOfFile)
		{
			...
			if(sel.IsRectangular()) SetEmptySelection(movePos);
			SetDragPosition(SelectionPosition(invalidPosition));

			STGMEDIUM medium = {0, {0}, 0};
			FORMATETC fmth = {CF_HDROP, NULL, DVASPECT_CONTENT, -1, TYMED_HGLOBAL };
			HRESULT hrHasFile = pIDataSource->GetData(&fmth, &medium);
			if (SUCCEEDED(hrHasFile) && medium.hGlobal) 
			{
				...
				if(cFiles > 0)
				{
					...
					DropAt(movePos, name, false, false);
				}
			}
		...
		*pdwEffect = EffectFromState(grfKeyState);
		...
		//MODDED: ADDED FOR WINXOUND (Rectangular selection)
