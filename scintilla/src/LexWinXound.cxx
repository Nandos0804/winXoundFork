...
static inline bool IsAValidNumber(const int ch);
...
static inline bool IsAValidNumber(const int ch) {
	//return (ch < 0x80) && (isalpha(ch) || ch == '.' || ch == '_' || ch == 60 || 
	//					   ch == 62 || ch == 47);
	return (ch == '.');
}
...
static void ColouriseCppDoc(unsigned int startPos, int length, int initStyle, 
	...
	for (; sc.More(); sc.Forward()) {
		...
		if (sc.state == SCE_WXOUND_OPERATOR) {
			...
			if (!IsAValidNumber(sc.ch)) {					//MODDED: !IsAWordChar
				sc.SetState(SCE_WXOUND_DEFAULT);
			}
	...
	for (unsigned int i = startPos; i < endPos; i++) {
		...
		if (style == SCE_WXOUND_DEFAULT) {
			if (ch == '{') {
				...
				levelNext--;
				//if(!iswordchar(styler.SafeGetCharAt(i + 1)))
				//	levelMinCurrent = levelNext;
				//levelMinCurrent = levelNext; //ADDED !!!
			}
		}


		//instr/endin
		if (style == SCE_WXOUND_WORD4) {
			if (styler.Match(i, "instr")) {
				levelNext++;
			} else if (styler.Match(i, "endin")) {
				levelNext--;
			}
		}
		...
		if (atEOL || (i == endPos-1)) {
			...
			int lev = levelUse | levelNext << 16;
			//if (visibleChars == 0 && foldCompact) //MODDED: WHITE LINES
			//	lev |= SC_FOLDLEVELWHITEFLAG;
			if (levelUse < levelNext)
				lev |= SC_FOLDLEVELHEADERFLAG;
			if (lev != styler.LevelAt(lineCurrent)) {
