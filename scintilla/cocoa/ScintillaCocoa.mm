...
//           slideBack: YES];
	...
	NSString* path = [[NSBundle mainBundle] pathForResource: @"drag" 
													 ofType: @"icns" inDirectory: nil];		
	NSImage* dragImage = [[[NSImage alloc] initWithContentsOfFile:path] autorelease];
	
	//	NSString* path = [[NSBundle mainBundle] pathForResource: @"mac_cursor_busy" ofType: @"png" inDirectory: nil];
	//    NSImage* dragImage = [[[NSImage alloc] initWithContentsOfFile: path] autorelease];
	
	//[content convertPoint: [lastMouseEvent locationInWindow] fromView: nil]
	
	NSPoint startPoint;
	startPoint.x = [content convertPoint: [lastMouseEvent locationInWindow] fromView: nil].x + 1;//selectionRectangle.origin.x;
	startPoint.y = [content convertPoint: [lastMouseEvent locationInWindow] fromView: nil].y + 28;//selectionRectangle.origin.y + selectionRectangle.size.height;
	
	[content dragImage: dragImage //image //dragImage 
					at: startPoint //startPoint //[lastMouseEvent locationInWindow] //startPoint
				offset: NSZeroSize
				 event: lastMouseEvent // Set in MouseMove.
			pasteboard: pasteboard
				source: content
...
//  return true;
	...
	//ScintillaWin contains this line: ???
	//SetDragPosition(SelectionPosition(invalidPosition));
	...
	if ([[pasteboard types] containsObject: NSFilenamesPboardType])
	{
	  ...
	  //Drop file path into text
	  if([files count] > 0)
	  {
		  //if(sel.IsRectangular()) SetEmptySelection(posDrag);
		  SetEmptySelection(posDrag);
			  
	      //::DropAt(SelectionPosition position, const char *value, bool moving, bool rectangular)
		  NSString* temp = [NSString stringWithFormat:@"\"%@\"", [files objectAtIndex:0]];
		  const char* droppedText = [temp UTF8String];
		  DropAt(posDrag, droppedText, false, false);
		
		  //const char* rawValue = [[files objectAtIndex:0] UTF8String];
		  //WndProc(SCI_INSERTTEXT, posDrag.Position(), (sptr_t) rawValue);
	  }
		
		
      //Notify about URI_DROPPED	
	  for (NSString* uri in files)
	    NotifyURIDropped([uri UTF8String]);
	}
	else
	{
	  ...
	  if (text.len > 0)
	  {
	    ...
	    //DropAt(posDrag, text.s, moving, text.rectangular);
		if(draggedText.s != nil)
		{
			//NSLog(@"!!!INTERNAL drag-drop data");
			DropAt(posDrag, draggedText.s, moving, draggedText.rectangular);
			draggedText.s = nil;
		}
		else 
		{
			//NSLog(@"!!!EXTERNAL drag-drop data");
			...
			NSString* data = [pasteboard stringForType: bestType];
			
			data = ConvertEOL(data);
			...
			if (data != nil)
			{
				...
				const char* newtext = (const char*) [data UTF8String];
				DropAt(posDrag, newtext, moving, false);
			}
			//DropAt(posDrag, text.s, moving, text.rectangular);
...
bool ScintillaCocoa::GetPasteboardData(NSPasteboard* board, SelectionText* selectedText)
{
  ...
  NSString* data = [board stringForType: bestType];
	...
	//MODDED FOR WINXOUND (check and convert Eols)
	data = ConvertEOL(data);
	//END_MOD
...
//ADDED FOR WINXOUND
NSString* ScintillaCocoa::ConvertEOL(NSString* data)
{
	...
	else if([data rangeOfString:@"\r"].location != NSNotFound)
	{
		// CR
...
bool ScintillaCocoa::ModifyScrollBars(int nMax, int nPage)
{
  ...
  if (horizontalScrollBarVisible)
    //pageSize = bounds.size.width; (this count also margins size)
	pageSize = GetTextRectangle().Width(); //MODDED (CORRECTION)
  else
    pageSize = scrollRange;
...
bool ScintillaCocoa::KeyboardInput(NSEvent* event)
{
	//ADDED FOR WINXOUND
	//ToDo: up/down arrow
	//if(pdoc->IsReadOnly()) return true;
	//END_MOD
...
//	  //ADDED FOR WINXOUND -> MOVED TO: ScintillaView->KeyDown event
//	  bool isRectangular = [TopContainer() getGeneralProperty:SCI_SELECTIONISRECTANGLE parameter:0];
//	  if(isRectangular && shift && alt == false)
//	  {	
//		  int curpos = [TopContainer() getGeneralProperty:SCI_GETANCHOR parameter:0];
//		  [TopContainer() setGeneralProperty: SCI_CLEARSELECTIONS	parameter: 0 value: 0];
//		  [TopContainer() setGeneralProperty:SCI_SETSELECTIONSTART parameter:curpos value:0];
//		  [TopContainer() setGeneralProperty:SCI_SETSELECTIONEND parameter:curpos value:0];	
//	  }
//	  //END_MOD
...
void ScintillaCocoa::MouseDown(NSEvent* event)
{
	...
	bool alt = ([event modifierFlags] & NSAlternateKeyMask) != 0; //ADDED FOR WINXOUND
	
	//ButtonDown(Point(location.x, location.y), (int) (time * 1000), shift, control, command);
	
	
	//MODDED FOR WINXOUND
	bool isRectangular = sel.IsRectangular(); //[TopContainer() getGeneralProperty:SCI_SELECTIONISRECTANGLE parameter:0];
	
	//If we click on margins area we must deselect any rectangular selection
	if(isRectangular && PointInSelMargin(location))
	{	
		//[TopContainer() setGeneralProperty: SCI_CLEARSELECTIONS parameter: 0 value: 0];
		WndProc(SCI_CLEARSELECTIONS, 0, 0);
		shift = false;
	}
	...
	if(isRectangular && shift && alt == false)
	{			
		int curpos = WndProc(SCI_GETANCHOR, 0, 0);
		//SetEmptySelection(curpos); //???
		WndProc(SCI_CLEARSELECTIONS, 0, 0);
		WndProc(SCI_SETSELECTIONSTART, curpos, 0);
		WndProc(SCI_SETSELECTIONEND, curpos, 0);
	}
	
	ButtonDown(Point(location.x, location.y), (int) (time * 1000), shift, control, alt);
	//END_MOD
...
void ScintillaCocoa::MouseWheel(NSEvent* event)
{
	...
	if (command && control)
	{
		...
		NSLog(@"Delta: %d - X:%f - Y:%f", delta, [event deltaX], [event deltaY]);
		//if (delta > 0)  //MODDED FOR WINXOUND (MAGIC MOUSE WHEEL AND TOUCHPAD BUG)
		if([event deltaY] > 0.333) 
			KeyCommand(SCI_ZOOMIN);
		else if ([event deltaY] < -0.333)
			KeyCommand(SCI_ZOOMOUT); //END MOD
	}
...
//ADDED FOR WINXOUND
NSInteger ScintillaCocoa::getSciTextRectangleWidth()
{
	return GetTextRectangle().Width();
}

void ScintillaCocoa::UpdateCaret()
{
	//NSLog(@"UpdateCaret");	
	caret.active = true;
	caret.on = true;
