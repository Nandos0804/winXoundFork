...
- (void) setCursor: (Window::Cursor) cursor
{
  [mCurrentCursor autorelease];
	...
	if(alternate) 
	{
		mCurrentCursor = [NSCursor crosshairCursor];
	}
	else 
	{	
	  switch (cursor)
 ...
 * the final text (e.g. composition of ^ and a to â).
 *
...
{
	//NSLog(@"flagsChanged");
	bool control = ([theEvent modifierFlags] & NSControlKeyMask) != 0;
	bool shift = ([theEvent modifierFlags] & NSShiftKeyMask) != 0;
	bool command = ([theEvent modifierFlags] & NSCommandKeyMask) != 0;
	bool alt = ([theEvent modifierFlags] & NSAlternateKeyMask) != 0;
	
	if(alt && !control && !shift && !command)
	{
		alternate = true;
		[self setCursor: Window::cursorHoriz];
	}
	else
	{
		alternate = false;
		[self setCursor: Window::cursorText];
	}
}
//END_MOD
...
- (void) keyDown: (NSEvent *) theEvent
{
	...
	bool isRectangular = [mOwner getGeneralProperty:SCI_SELECTIONISRECTANGLE parameter:0];
	NSInteger selectionStart = [mOwner getGeneralProperty:SCI_GETSELECTIONSTART parameter:0];
	NSInteger selectionEnd = [mOwner getGeneralProperty:SCI_GETSELECTIONEND parameter:0];
	bool isSelected = selectionStart - selectionEnd;	
	
	
	
	//If selection is Rectangular we must deselect the area before to continue
	//Otherwise the Rectangular selection remains selected
	if(isRectangular && shift && alt == false)
	{	
		  int curpos = [mOwner getGeneralProperty:SCI_GETANCHOR parameter:0];
		  [mOwner setGeneralProperty: SCI_CLEARSELECTIONS parameter: 0 value: 0];
		  [mOwner setGeneralProperty:SCI_SETSELECTIONSTART parameter:curpos value:0];
		  [mOwner setGeneralProperty:SCI_SETSELECTIONEND parameter:curpos value:0];		
	}
	
	
	
	
	if(alt && shift && [theEvent keyCode] == 123 && 
	   !isRectangular && !control && !command) //ALT+SHIFT+LEFT
	{
		[mOwner setGeneralProperty:SCI_WORDLEFTEXTEND parameter:0 value:0];
		consumed = true;
	}
	else if(alt && shift && [theEvent keyCode] == 124 && 
			!isRectangular && !control && !command) //ALT+SHIFT+RIGHT
	{
		//SCI_WORDRIGHTEXTEND //SCI_WORDRIGHTENDEXTEND
		[mOwner setGeneralProperty:SCI_WORDRIGHTENDEXTEND parameter:0 value:0];
		consumed = true;
	}
	else if(alt && shift && [theEvent keyCode] == 126 && 
			!isRectangular && !control && !command) //ALT+SHIFT+UP
	{
		NSInteger anchorPosition = [mOwner getGeneralProperty:SCI_GETANCHOR parameter:0];
		NSInteger line = [mOwner getGeneralProperty:SCI_LINEFROMPOSITION 
										  parameter: [mOwner getGeneralProperty:SCI_GETCURRENTPOS parameter:0]];
		NSInteger lineStart = [mOwner getGeneralProperty:SCI_POSITIONFROMLINE parameter:line];
		//NSInteger lineEnd = [mOwner getGeneralProperty:SCI_GETLINEENDPOSITION parameter:line];
		
		if(!isSelected && selectionStart > lineStart)
		{
			[mOwner setGeneralProperty:SCI_HOMEEXTEND parameter:0 value:0];
		}
		else if (!isSelected && selectionStart == lineStart)
		{
			[mOwner setGeneralProperty:SCI_LINEUPEXTEND parameter:0 value:0];
			[mOwner setGeneralProperty:SCI_HOMEEXTEND parameter:0 value:0];
		}
		else if(selectionEnd > anchorPosition  &&
				[mOwner getGeneralProperty:SCI_LINEFROMPOSITION parameter:anchorPosition] == line)
		{
			[mOwner setGeneralProperty:SCI_SETSELECTIONSTART parameter:anchorPosition value:0];
			[mOwner setGeneralProperty:SCI_SETSELECTIONEND parameter:anchorPosition value:0];
		}
		else if(selectionStart < anchorPosition)
		{
			[mOwner setGeneralProperty:SCI_LINEUPEXTEND parameter:0 value:0];
			[mOwner setGeneralProperty:SCI_HOMEEXTEND parameter:0 value:0];
		}
		...
		{
			[mOwner setGeneralProperty:SCI_LINEUPEXTEND parameter:0 value:0];
			[mOwner setGeneralProperty:SCI_LINEENDEXTEND parameter:0 value:0];
		}
		
		consumed = true;
	}
	else if(alt && shift && [theEvent keyCode] == 125 && 
			!isRectangular && !control && !command) //ALT+SHIFT+DOWN
	{		
		NSInteger anchorPosition = [mOwner getGeneralProperty:SCI_GETANCHOR parameter:0];
		NSInteger line = [mOwner getGeneralProperty:SCI_LINEFROMPOSITION 
										  parameter: [mOwner getGeneralProperty:SCI_GETCURRENTPOS parameter:0]];
		...
		if(!isSelected && selectionStart < lineEnd)
		{
			[mOwner setGeneralProperty:SCI_LINEENDEXTEND parameter:0 value:0];
		}
		else if(!isSelected && selectionStart == lineEnd)
		{
			[mOwner setGeneralProperty:SCI_LINEDOWNEXTEND parameter:0 value:0];
			[mOwner setGeneralProperty:SCI_LINEENDEXTEND parameter:0 value:0];
		}
		else if(selectionStart < anchorPosition &&
				[mOwner getGeneralProperty:SCI_LINEFROMPOSITION parameter:anchorPosition] == line)
		...
		else if(anchorPosition < selectionEnd)
		{
			[mOwner setGeneralProperty:SCI_LINEDOWNEXTEND parameter:0 value:0];
			[mOwner setGeneralProperty:SCI_LINEENDEXTEND parameter:0 value:0];
		}
	...
	else if (alt && [theEvent keyCode] == 123 &&
		!shift && !control && !command) //ALT+LEFT
	{
		[mOwner setGeneralProperty:SCI_WORDLEFT parameter:0 value:0];
		consumed = true;
	}
	else if (alt && [theEvent keyCode] == 124 &&
			 !shift && !control && !command) //ALT+RIGHT
	{
		//SCI_WORDRIGHT //SCI_WORDRIGHTEND
		[mOwner setGeneralProperty:SCI_WORDRIGHTEND parameter:0 value:0];
		consumed = true;
	}
	
	else if (alt && [theEvent keyCode] == 126 &&
			 !shift && !control && !command) //ALT+UP
	{
		NSInteger line = [mOwner getGeneralProperty:SCI_LINEFROMPOSITION 
										  parameter: [mOwner getGeneralProperty:SCI_GETCURRENTPOS parameter:0]];
		NSInteger start = [mOwner getGeneralProperty:SCI_POSITIONFROMLINE parameter:line];
		
		if([mOwner getGeneralProperty:SCI_GETCURRENTPOS parameter:0] == start)
		{
			[mOwner setGeneralProperty:SCI_LINEUP parameter:0 value:0];
			[mOwner setGeneralProperty:SCI_HOME parameter:0 value:0];
		}
		else 
		{
			[mOwner setGeneralProperty:SCI_HOME parameter:0 value:0];
	...
	else if (alt && [theEvent keyCode] == 125 &&
			 !shift && !control && !command) //ALT+DOWN
	{
		NSInteger line = [mOwner getGeneralProperty:SCI_LINEFROMPOSITION 
										  parameter: [mOwner getGeneralProperty:SCI_GETCURRENTPOS parameter:0]];
		NSInteger lineEnd = [mOwner getGeneralProperty:SCI_GETLINEENDPOSITION parameter:line];
		
		if([mOwner getGeneralProperty:SCI_GETCURRENTPOS parameter:0] == lineEnd)
		{
			[mOwner setGeneralProperty:SCI_LINEDOWN parameter:0 value:0];
			[mOwner setGeneralProperty:SCI_LINEEND parameter:0 value:0];
		}
	...
	else if (command && [theEvent keyCode] == 123 &&
			 !shift && !control && !alt) //CMD+LEFT
	{
		[mOwner setGeneralProperty:SCI_HOME parameter:0 value:0];
		consumed = true;
	}
	else if (command && [theEvent keyCode] == 124 &&
			 !shift && !control && !alt) //CMD+RIGHT
	{
		[mOwner setGeneralProperty:SCI_LINEEND parameter:0 value:0];
		consumed = true;
	}
	else if (command && shift && [theEvent keyCode] == 123) //CMD+SHIFT+LEFT
	{
		NSInteger line = [mOwner getGeneralProperty:SCI_LINEFROMPOSITION 
										  parameter: [mOwner getGeneralProperty:SCI_GETCURRENTPOS parameter:0]];
		NSInteger lineEnd = [mOwner getGeneralProperty:SCI_GETLINEENDPOSITION parameter:line];
	...
	else if (command && shift && [theEvent keyCode] == 124) //CMD+SHIFT+RIGHT
	{
		NSInteger line = [mOwner getGeneralProperty:SCI_LINEFROMPOSITION 
										  parameter: [mOwner getGeneralProperty:SCI_GETCURRENTPOS parameter:0]];
		NSInteger start = [mOwner getGeneralProperty:SCI_POSITIONFROMLINE parameter:line];
	...
	else if (command && shift && [theEvent keyCode] == 126) //CMD+SHIFT+UP
	{
		if([mOwner getGeneralProperty:SCI_GETCURRENTPOS parameter:0] == 
		   [mOwner getGeneralProperty:SCI_GETTEXTLENGTH parameter:0])
		{
			[mOwner setGeneralProperty:SCI_DOCUMENTEND parameter:0 value:0];	
			[mOwner setGeneralProperty:SCI_DOCUMENTSTARTEXTEND parameter:0 value:0];	
		}
		else 
		{
			[mOwner setGeneralProperty:SCI_DOCUMENTSTARTEXTEND parameter:0 value:0];
	...
	else if (command && shift && [theEvent keyCode] == 125) //CMD+SHIFT+DOWN
	{
		if([mOwner getGeneralProperty:SCI_GETCURRENTPOS parameter:0] == 0)
		{
			[mOwner setGeneralProperty:SCI_DOCUMENTSTART parameter:0 value:0];	
			[mOwner setGeneralProperty:SCI_DOCUMENTENDEXTEND parameter:0 value:0];	
		}
		else 
		{
			[mOwner setGeneralProperty:SCI_DOCUMENTENDEXTEND parameter:0 value:0];	
	...
	//If not consumed send to ScintillaCocoa
	if(consumed == false)
	{
	...
	//SEND ESCAPE-KEYDOWN NOTIFICATION //Escape = 53
	if([theEvent keyCode] == 53) 
		[mOwner sendNotification: @"SCIEscapeKeyDown"];
...
{	
	//[mOwner setGeneralProperty: SCI_SETCURRENTPOS parameter: 0];
	
	NSPoint location = [self convertPoint:[theEvent locationInWindow] fromView:nil];
	int position = [mOwner getGeneralProperty: SCI_POSITIONFROMPOINT 
									parameter:location.x
										extra:location.y];

	if([self selectedRange].length == 0 ||
	   position < [self selectedRange].location ||
	   position > ([self selectedRange].location + [self selectedRange].length))
	{

		//NSLog(@"%d", position);

		[mOwner setGeneralProperty:SCI_GOTOPOS parameter:position value:0];
		mOwner.backend->ShowCaret();
...
//		//int currentPosition = [mOwner getGeneralProperty: SCI_GETCURRENTPOS parameter: 0];
//		int wordStart = [mOwner getGeneralProperty: SCI_WORDSTARTPOSITION parameter: 1];
//		int wordEnd = [mOwner getGeneralProperty: SCI_WORDENDPOSITION parameter: 1];
//		
//		 [mOwner setGeneralProperty: SCI_SETSELECTIONSTART
//						  parameter: wordStart
//							  value: 0];
...
//							  value: 0];
	...
	[mOwner sendNotification: @"SCIRightMouseDown"];
	//[[NSNotificationCenter defaultCenter] postNotificationName:@"SCIRightMouseDown" object:self];
...
- (void) mouseDown: (NSEvent *) theEvent  
{
    ...
    //the "mOwner.backend->MouseDown(theEvent)" method;
	if(command && alt 
	   && !control && !shift) 
	{
		//NSLog(@"CMD+ALT Pressed");
		//[mOwner sendNotification: @"SCILeftMouseSelectQuotes"];
		...
		[center postNotificationName: @"SCILeftMouseSelectQuotes" 
							  object: mOwner
							userInfo: [NSDictionary dictionaryWithObject:[NSString stringWithFormat: @"%d", position]
																  forKey:@"position"]];
	...
	mOwner.backend->MouseDown(theEvent); //Original line
	
	
	//MODDED FOR WINXOUND:
	[mOwner sendNotification: @"SCILeftMouseDown"];
	
	if(command 
	   && !control && !shift && !alt) 
	{
		[mOwner sendNotification: @"SCILeftMouseDownHyperlinks"];
	}
...
- (void) selectAll: (id) sender
{
	...
	[mOwner setGeneralProperty:SCI_SELECTALL parameter:0 value:0];
}
...
{
	//NSLog(@"delete");
	if([[mOwner selectedString] length] > 0)
	{
...
{
	//mOwner.backend->Undo();
	[mOwner setGeneralProperty:SCI_UNDO parameter:0 value:0];
}

- (void) redo: (id) sender
{
	//mOwner.backend->Redo();
	[mOwner setGeneralProperty:SCI_REDO parameter:0 value:0];
}

- (BOOL) validateMenuItem:(NSMenuItem *)menuItem
{
	//NSLog(@"ScintillaView validateMenuItem");
	
	if([[menuItem title] isEqualToString:@"Undo"])
	{
		//NSLog(@"VALIDATE MENU UNDO");
		return mOwner.backend->CanUndo();
		//return [textEditor getCanUndo];
	}
	if([[menuItem title] isEqualToString:@"Redo"])
	{
		//NSLog(@"VALIDATE MENU REDO");
		return mOwner.backend->CanRedo();
		//return [textEditor getCanRedo];
	}
	if([[menuItem title] isEqualToString:@"Cut"])
	{
		//NSLog(@"VALIDATE MENU CUT");
		return ([[mOwner selectedString] length] > 0);
	}
	if([[menuItem title] isEqualToString:@"Copy"])
	{
		//NSLog(@"VALIDATE MENU COPY");
		return ([[mOwner selectedString] length] > 0);
	}
	if([[menuItem title] isEqualToString:@"Paste"])
	{
		return mOwner.backend->CanPaste();
		//return [textEditor getCanPaste];
	}
      ...
      switch (scn->nmhdr.code)
      {
        ...
        case SCN_MODIFIED:
        {
          ...
          // There can be more than one modification carried by one notification.
			...
			{
				//[editor sendNotification: @"SCIModContainer"];
				NSNotificationCenter* center = [NSNotificationCenter defaultCenter];
				[center postNotificationName: @"SCIModContainer" 
									  object: editor
									userInfo: [NSDictionary dictionaryWithObject:[NSString stringWithFormat: @"%d", scn->token]
																		  forKey:@"token"]];
			}
			//END_MOD
        ...
        case SCN_ZOOM:
        {
	      ...
	      //MODDED FOR WINXOUND
          //float zoom = [editor getGeneralProperty: SCI_GETZOOM parameter: 0]; //MOD: REM
          //int fontSize = [editor getGeneralProperty: SCI_STYLEGETSIZE parameter: STYLE_DEFAULT]; //MOD: REM
          //float factor = (zoom / fontSize) + 1; //MOD: REM
          //[editor->mInfoBar notify: IBNZoomChanged message: nil location: NSZeroPoint value: factor]; //MOD: REM
		  //ADDED FOR WINXOUND
		  [editor sendNotification: @"SCIZoomChanged"];
		  //END_MOD
          break;
        }
        case SCN_UPDATEUI:
...
//  return result;
	...
	/////////////////////
	//MODDED FOR WINXOUND
	//NSLog(@"ScintillaView setHorizontalScrollRange");
	
	BOOL result = NO;
	BOOL hideScroller = (page >= range) && ([self getGeneralProperty:SCI_GETXOFFSET parameter:0] == 0);
	...
	if ([mHorizontalScroller isHidden] != hideScroller)
	{
		...
		//////////////
		if (!hideScroller)
			[mHorizontalScroller setFloatValue: 0];
		//////////////
		[self layout];
	}
	...
	{
		//////////////
		float position = [mHorizontalScroller floatValue];
		mBackend->DoScroll(position, [mHorizontalScroller hitPart], true);
		//////////////
		...
		if(page < range)
		{
			...
			if (currentProportion != newProportion)
			{
				...
				//if([self getGeneralProperty:SCI_GETXOFFSET parameter:0] < 1)
				{
					[mHorizontalScroller setKnobProportion: newProportion];
					//NSLog(@"Proportion: %f", newProportion);
				}
...
- (NSString*) selectedString
{
  ...
  if (length > 0)
  {
    ...
    try
    {
      ...
      //mBackend->WndProc(SCI_SETSAVEPOINT, 0, 0);  //MODDED FOR WINXOUND (REM)
...
//ADDED FOR WINXOUND
- (NSInteger) getTextRectangleWidth
{
	return mBackend->getSciTextRectangleWidth();
}
- (NSRange) selectedRange
{
	int begin = [self getGeneralProperty: SCI_GETSELECTIONSTART parameter: 0];
	int end = [self getGeneralProperty: SCI_GETSELECTIONEND parameter: 0];
	return NSMakeRange(begin, end - begin);
}
