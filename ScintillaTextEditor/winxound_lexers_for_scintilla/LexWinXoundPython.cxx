	...
	for (; sc.More(); sc.Forward()) {
		...
		if (sc.state == SCE_P_DEFAULT) {
			if (IsADigit(sc.ch) || (sc.ch == '.' && IsADigit(sc.chNext))) {
				...
				unsigned int nextIndex = 0;
				int curstate = 
					GetPyStringState(styler, sc.currentPos, &nextIndex, allowedLiterals);
				if(curstate != SCE_P_TRIPLE)
				{
...
LexerModule lmWinXoundPython(998, ColourisePyDoc, "winxoundpython", FoldPyDoc,
					 pythonWordListDesc);
