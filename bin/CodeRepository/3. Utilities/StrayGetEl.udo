...
DESCRIPTION
Returns the startindex and the endindex (= the first space after the element) for ielindex in String. If iselstart returns -1, the element has not been found. By default, the seperators between the elements are spaces and tabs. Others seperators can be defined by their ASCII code number.
...
Output:
iselst - string index (as to resolve with strsub) for the beginning of the element. If ielindx is out of range, -1 is returned
iselst - string index for the end of the element (= the first character after the element)
